...
#define DUNGEON_CELL_MASK_WORD_BITS 64
...
#define DUNGEON_FAMILIAR_BASIC_RANGED_FOLLOW_MIN_DISTANCE 2
#define DUNGEON_FAMILIAR_BASIC_RANGED_FOLLOW_MAX_DISTANCE 4
#define DUNGEON_FAMILIAR_BASIC_RANGED_ATTACK_MIN_DISTANCE 2
#define DUNGEON_FAMILIAR_BASIC_RANGED_ATTACK_MAX_DISTANCE 6
#define DUNGEON_FAMILIAR_TRANSITION_SPAWN_MIN_PLAYER_DISTANCE 2
...
#define DUNGEON_FAMILIAR_BAT_DASH_RANGE 5
...
#define DUNGEON_BEHOLDER_ALIGNED_RANGED_RANGE 10
...
#define DUNGEON_TILE_FX_CAPACITY DUNGEON_MAX_UNITS
#define DUNGEON_BEETLE_EXPLOSION_FX_ROW 0
#define DUNGEON_BEETLE_EXPLOSION_FX_COL_START 8
#define DUNGEON_BEETLE_EXPLOSION_FX_FRAME_COUNT 3
#define DUNGEON_BEETLE_EXPLOSION_FX_FPS (DUNGEON_SPRITE_ANIM_FPS * 2.0f)
...
#define DUNGEON_PLAYER_PANEL_GAP 8.0f
...
typedef struct {
    const char *label;
    FAMILIAR_TURN_COMMAND command;
    bool requires_living_familiar;
} Dungeon_Familiar_Command_Def;
...
_Static_assert((sizeof(game_dungeon_familiar_command_defs) /
                sizeof(game_dungeon_familiar_command_defs[0])) == DUNGEON_FAMILIAR_COMMAND_COUNT,
               "Familiar command definition count must match DUNGEON_FAMILIAR_COMMAND_COUNT");

typedef enum {
    UNIT_AI_BASIC_MELEE,
    UNIT_AI_BASIC_RANGED,
    UNIT_AI_DEFEND_RANGED,
    UNIT_AI_ALIGNED_RANGED,
    UNIT_AI_DASH,
    UNIT_AI_DASH_THROUGH,
    UNIT_AI_TURRET,
...
typedef struct {
    ...
    float attack_anim_dir_x;
    float attack_anim_dir_y;
    float attack_anim_delay;
    float attack_anim_elapsed;
    UNIT_ART_KIND kind;
    u8 orientation;
    bool is_friendly;
    Unit_Stats stats;
...
typedef struct {
    ...
    u64 dungeon_visible_mask_by_depth[DUNGEON_MAX_FLOOR_DEPTH][DUNGEON_CELL_MASK_WORD_COUNT];
    ...
    Dungeon_Tile_Fx tile_fx[DUNGEON_TILE_FX_CAPACITY];
    u16 tile_fx_cursor;
    ...
    float player_attack_anim_dir_x;
    float player_attack_anim_dir_y;
    float player_attack_anim_delay;
    float player_attack_anim_elapsed;
...
static i32 game_dungeon_cell_index(i32 x, i32 y)
{
    return y * DUNGEON_COL_COUNT + x;
}

static void game_dungeon_clear_floor_reveal_cache(Game *game)
{
    memset(game->dungeon_visible_mask_by_depth, 0, sizeof(game->dungeon_visible_mask_by_depth));
    memset(game->dungeon_visible_mask_valid_by_depth, 0,
           sizeof(game->dungeon_visible_mask_valid_by_depth));
...
static void game_dungeon_store_revealed_map_for_depth(Game *game, u32 depth)
{
    ...
    u64 *visible_mask = game->dungeon_visible_mask_by_depth[depth];
    u64 *explored_mask = game->dungeon_explored_mask_by_depth[depth];
    memset(visible_mask, 0, sizeof(game->dungeon_visible_mask_by_depth[depth]));
    memset(explored_mask, 0, sizeof(game->dungeon_explored_mask_by_depth[depth]));

    for (i32 y = 0; y < DUNGEON_ROW_COUNT; y++) {
        for (i32 x = 0; x < DUNGEON_COL_COUNT; x++) {
            i32 cell_idx = game_dungeon_cell_index(x, y);
            i32 word_idx = cell_idx / DUNGEON_CELL_MASK_WORD_BITS;
            i32 bit_idx = cell_idx % DUNGEON_CELL_MASK_WORD_BITS;
            u64 mask_bit = 1ull << bit_idx;

            if (game->dungeon_visible[y][x] != 0)
                visible_mask[word_idx] |= mask_bit;
            if (game->dungeon_explored[y][x] != 0)
                explored_mask[word_idx] |= mask_bit;
        }
    }

    game->dungeon_visible_mask_valid_by_depth[depth] = 1;
    game->dungeon_explored_mask_valid_by_depth[depth] = 1;
}

static bool game_dungeon_restore_revealed_map_for_depth(Game *game, u32 depth)
{
    if (depth >= DUNGEON_MAX_FLOOR_DEPTH)
        return false;
    if (game->dungeon_visible_mask_valid_by_depth[depth] == 0)
        return false;
    if (game->dungeon_explored_mask_valid_by_depth[depth] == 0)
        return false;

    const u64 *visible_mask = game->dungeon_visible_mask_by_depth[depth];
    const u64 *explored_mask = game->dungeon_explored_mask_by_depth[depth];
    memset(game->dungeon_explored, 0, sizeof(game->dungeon_explored));
    game_dungeon_clear_visible_cells(game);
    ...
    for (i32 y = 0; y < DUNGEON_ROW_COUNT; y++) {
        for (i32 x = 0; x < DUNGEON_COL_COUNT; x++) {
            ...
            i32 bit_idx = cell_idx % DUNGEON_CELL_MASK_WORD_BITS;
            u64 mask_bit = 1ull << bit_idx;
            game->dungeon_visible[y][x] = (visible_mask[word_idx] & mask_bit) != 0 ? 1 : 0;
            game->dungeon_explored[y][x] = (explored_mask[word_idx] & mask_bit) != 0 ? 1 : 0;
        }
    }
                                    ...
                                    const char *label)
{
    ...
    const char *prefix = *in_out_has_status ? ", " : "";
    snprintf(buffer + len, buffer_cap - len, "%s%s", prefix, label);
    *in_out_has_status = true;
}

static bool game_dungeon_build_unit_status_text(const Dungeon_Unit *unit, char *buffer,
                                                u32 buffer_cap)
{
    assert(unit != 0);
    assert(buffer != 0);
    ...
    if (unit->is_burning) {
        ...
        snprintf(burn_label, sizeof(burn_label), "Burn x%d (%dt)", burn_stacks,
                 (i32)unit->burn_turns_remaining);
    ...
    if (unit->is_webbed) {
        ...
        snprintf(webbed_label, sizeof(webbed_label), "Webbed (%dt)",
                 (i32)unit->webbed_turns_remaining);
...
static bool game_dungeon_build_player_status_text(const Game *game, char *buffer, u32 buffer_cap)
{
    assert(game != 0);
    assert(buffer != 0);
    assert(buffer_cap > 0);
...
static Unit_Stats game_get_unit_base_stats(UNIT_ART_KIND kind)
{
    ...
    case UNIT_ART_GRIFFON:
        return game_make_unit_stats(7, 5);
    case UNIT_ART_MAN_EATER:
...
static UNIT_AI_KIND game_get_unit_ai_kind(UNIT_ART_KIND kind)
{
    switch (kind) {
    case UNIT_ART_SPIDER:
                                  ...
                                  bool is_friendly)
{
    ...
    game->units[game->unit_count++] = (Dungeon_Unit){
        ...
        .attack_anim_dir_x = 0.0f,
        .attack_anim_dir_y = 0.0f,
        .attack_anim_delay = 0.0f,
        .attack_anim_elapsed = DUNGEON_ATTACK_ANIM_DURATION,
        .kind = kind,
...
static void game_dungeon_add_defeated_unit_visual(Game *game, const Dungeon_Unit *unit)
{
    ...
    u8 visual_idx = game->defeated_unit_visual_cursor;
    game->defeated_unit_visual_cursor =
        (u8)((game->defeated_unit_visual_cursor + 1) % DUNGEON_MAX_UNITS);

    game->defeated_unit_visuals[visual_idx] = (Dungeon_Defeated_Unit_Visual){
        .x = unit->x,
        .y = unit->y,
        .move_anim_from_x = unit->move_anim_from_x,
        .move_anim_from_y = unit->move_anim_from_y,
        .move_anim_elapsed = unit->move_anim_elapsed,
        .kind = unit->kind,
...
static void game_dungeon_remove_unit_at(Game *game, i32 unit_idx)
{
    ...
    Dungeon_Unit removed_unit = game->units[unit_idx];
    game_dungeon_add_defeated_unit_visual(game, &removed_unit);
...
static void game_dungeon_spawn_tile_fx(Game *game, i32 x, i32 y, i32 row, i32 col_start,
                                       i32 frame_count, float fps, float start_delay)
{
    assert(game != 0);
    assert(game_dungeon_cell_in_bounds(x, y));
    assert(row >= 0 && row < FX_ART_ROW_COUNT);
    assert(col_start >= 0 && col_start < FX_ART_COL_COUNT);
    assert(frame_count > 0);
    assert(col_start + frame_count <= FX_ART_COL_COUNT);
    assert(fps > 0.0f);

    i32 fx_idx = (i32)(game->tile_fx_cursor % DUNGEON_TILE_FX_CAPACITY);
    game->tile_fx_cursor = (u16)((game->tile_fx_cursor + 1) % DUNGEON_TILE_FX_CAPACITY);

    game->tile_fx[fx_idx] = (Dungeon_Tile_Fx){
        .x = (i16)x,
        .y = (i16)y,
        .row = (u8)row,
        .col_start = (u8)col_start,
        .frame_count = (u8)frame_count,
        .fps = fps,
...
static void game_dungeon_spawn_beetle_explosion_fx(Game *game, i32 x, i32 y, float start_delay)
{
    assert(game != 0);

    game_dungeon_spawn_tile_fx(
        game, x, y, DUNGEON_BEETLE_EXPLOSION_FX_ROW, DUNGEON_BEETLE_EXPLOSION_FX_COL_START,
        DUNGEON_BEETLE_EXPLOSION_FX_FRAME_COUNT, DUNGEON_BEETLE_EXPLOSION_FX_FPS, start_delay);
}
                                              ...
                                              DAMAGE_KIND damage_kind, bool show_damage_fx)
{
    ...
    if (defeated_unit.kind == UNIT_ART_BEETLE) {
        float explosion_delay = game_dungeon_get_unit_move_anim_remaining(&defeated_unit);
        game_dungeon_trigger_beetle_explosion_at(game, &defeated_unit, explosion_delay);
    }
...
static bool
game_dungeon_build_distance_field(const Game *game, const i16 *goal_x, const i16 *goal_y,
                                  i32 goal_count,
                                  i16 out_distance[DUNGEON_ROW_COUNT][DUNGEON_COL_COUNT])
{
    ...
    for (i32 goal_idx = 0; goal_idx < goal_count; goal_idx++) {
        ...
        if (out_distance[source_y][source_x] == 0)
            continue;

        out_distance[source_y][source_x] = 0;
    ...
    while (head < tail) {
        ...
        i32 next_distance = out_distance[y][x] + 1;
...
static bool
game_dungeon_build_player_distance_field(const Game *game,
                                         i16 out_distance[DUNGEON_ROW_COUNT][DUNGEON_COL_COUNT])
{
...
static bool game_dungeon_find_nearest_unoccupied_floor_cell(const Game *game, i32 origin_x,
                                                            i32 origin_y, i32 min_player_distance,
                                                            i32 *out_x, i32 *out_y)
{
    assert(game != 0);
    assert(out_x != 0);
    ...
    for (i32 y = 0; y < DUNGEON_ROW_COUNT; y++) {
        for (i32 x = 0; x < DUNGEON_COL_COUNT; x++) {
            ...
            if (min_player_distance > 0) {
                i32 player_distance_x = x - game->player_x;
                if (player_distance_x < 0)
                    player_distance_x = -player_distance_x;
                i32 player_distance_y = y - game->player_y;
                if (player_distance_y < 0)
                    player_distance_y = -player_distance_y;
                i32 player_distance = player_distance_x + player_distance_y;
...
static u8 game_dungeon_capture_living_familiars(const Game *game,
                                                Dungeon_Unit out_familiars[DUNGEON_MAX_UNITS])
{
    assert(game != 0);
    assert(out_familiars != 0);

    u8 familiar_count = 0;
    for (u8 idx = 0; idx < game->unit_count; idx++) {
        const Dungeon_Unit *unit = &game->units[idx];
        if (!unit->is_friendly)
            continue;

        if (familiar_count >= DUNGEON_MAX_UNITS)
            break;
        out_familiars[familiar_count++] = *unit;
    }
                                                       ...
                                                       u8 familiar_count, bool heal_to_full)
{
    ...
    for (u8 familiar_idx = 0; familiar_idx < familiar_count; familiar_idx++) {
        if (game->unit_count >= DUNGEON_MAX_UNITS)
            break;

        const Dungeon_Unit *saved = &familiars[familiar_idx];
        i32 spawn_x = 0;
        i32 spawn_y = 0;
        if (!game_dungeon_find_nearest_unoccupied_floor_cell(
                game, game->player_x, game->player_y,
                DUNGEON_FAMILIAR_TRANSITION_SPAWN_MIN_PLAYER_DISTANCE, &spawn_x, &spawn_y)) {
            break;
        }

        u8 orientation = saved->orientation;
        if (orientation >= UNIT_ART_ORIENTATION_COUNT)
            orientation = PLAYER_START_ORIENTATION;

        game_dungeon_add_unit(game, spawn_x, spawn_y, saved->kind, orientation, true);
...
static float game_dungeon_get_unit_move_anim_remaining(const Dungeon_Unit *unit)
{
    ...
    if (!unit->move_anim_active)
        return 0.0f;
    return max(0.0f, DUNGEON_MOVE_ANIM_DURATION - unit->move_anim_elapsed);
}

static float game_dungeon_get_player_move_anim_remaining(const Game *game)
{
    assert(game != 0);

    if (!game->player_move_anim_active)
        return 0.0f;
    return max(0.0f, DUNGEON_MOVE_ANIM_DURATION - game->player_move_anim_elapsed);
}

static float game_dungeon_get_attack_anim_t(float elapsed)
{
    if (DUNGEON_ATTACK_ANIM_DURATION <= 0.0f)
        return 1.0f;
    return clamp(elapsed / DUNGEON_ATTACK_ANIM_DURATION, 0.0f, 1.0f);
}
...
static void game_dungeon_begin_unit_move_animation(Dungeon_Unit *unit, i32 start_x, i32 start_y)
{
    ...
    unit->attack_anim_dir_x = 0.0f;
    unit->attack_anim_dir_y = 0.0f;
    unit->attack_anim_delay = 0.0f;
    unit->attack_anim_elapsed = DUNGEON_ATTACK_ANIM_DURATION;
    unit->attack_anim_active = false;
}
...
static float game_dungeon_get_attack_anim_dash_amount(float t)
{
    t = clamp(t, 0.0f, 1.0f);
    if (t <= 0.5f)
        return game_dungeon_get_move_anim_eased_t(t / 0.5f);
    return 1.0f - game_dungeon_get_move_anim_eased_t((t - 0.5f) / 0.5f);
}

static void game_dungeon_get_attack_anim_direction(i32 start_x, i32 start_y, i32 target_x,
                                                   i32 target_y, float *out_dir_x, float *out_dir_y)
{
    assert(out_dir_x != 0);
    assert(out_dir_y != 0);
...
static void game_dungeon_begin_unit_attack_animation(Dungeon_Unit *unit, i32 start_x, i32 start_y,
                                                     i32 target_x, i32 target_y, float start_delay)
{
    assert(unit != 0);

    float dir_x = 0.0f;
    float dir_y = 0.0f;
    game_dungeon_get_attack_anim_direction(start_x, start_y, target_x, target_y, &dir_x, &dir_y);
    if (dir_x == 0.0f && dir_y == 0.0f)
        return;

    unit->attack_anim_dir_x = dir_x;
    unit->attack_anim_dir_y = dir_y;
    unit->attack_anim_delay = max(0.0f, start_delay);
    unit->attack_anim_elapsed = 0.0f;
    unit->attack_anim_active = true;
}

static void game_dungeon_begin_player_attack_animation(Game *game, i32 start_x, i32 start_y,
                                                       ...
                                                       float start_delay)
{
    ...
    game->player_attack_anim_dir_x = dir_x;
    game->player_attack_anim_dir_y = dir_y;
    game->player_attack_anim_delay = max(0.0f, start_delay);
    game->player_attack_anim_elapsed = 0.0f;
...
static void game_dungeon_spawn_web_projectile(Game *game, i32 start_x, i32 start_y, i32 target_x,
                                              i32 target_y, bool target_is_player,
                                              u16 target_unit_id, float start_delay, Color color)
{
    assert(game != 0);
    if (target_is_player)
        assert(target_unit_id == 0);
...
static bool game_dungeon_cells_are_aligned(i32 a_x, i32 a_y, i32 b_x, i32 b_y,
                                           bool include_diagonals)
{
    ...
    return dx == 0 || dy == 0 || (include_diagonals && dx == dy);
}

static bool game_dungeon_cells_are_line_aligned(i32 a_x, i32 a_y, i32 b_x, i32 b_y)
{
    return game_dungeon_cells_are_aligned(a_x, a_y, b_x, b_y, false);
}

static bool game_dungeon_get_aligned_step_towards_cell(i32 start_x, i32 start_y, i32 target_x,
                                                       i32 target_y, bool include_diagonals,
                                                       i32 *out_step_x, i32 *out_step_y)
{
    assert(out_step_x != 0);
    assert(out_step_y != 0);
    ...
    i32 abs_dx = dx < 0 ? -dx : dx;
    i32 abs_dy = dy < 0 ? -dy : dy;
    if (!(abs_dx == 0 || abs_dy == 0 || (include_diagonals && abs_dx == abs_dy)))
        return false;

    *out_step_x = dx > 0 ? 1 : (dx < 0 ? -1 : 0);
    *out_step_y = dy > 0 ? 1 : (dy < 0 ? -1 : 0);
    return true;
}

static bool game_dungeon_get_line_step_towards_cell(i32 start_x, i32 start_y, i32 target_x,
                                                    i32 target_y, i32 *out_step_x, i32 *out_step_y)
{
    return game_dungeon_get_aligned_step_towards_cell(start_x, start_y, target_x, target_y, false,
                                                      out_step_x, out_step_y);
}
...
static bool game_dungeon_unit_can_attack_cell(const Game *game, const Dungeon_Unit *unit,
                                              i32 target_x, i32 target_y)
{
    assert(game != 0);
    assert(unit != 0);

    return game_dungeon_unit_can_see_cell(game, unit->x, unit->y, target_x, target_y);
}

static i16 game_dungeon_get_player_distance_for_threat(
    const Game *game, const i16 player_distance[DUNGEON_ROW_COUNT][DUNGEON_COL_COUNT],
    bool player_distance_valid, i32 x, i32 y)
{
    i16 distance = DUNGEON_PATH_UNREACHABLE - 1;
    if (player_distance_valid && game_dungeon_cell_in_bounds(x, y)) {
        distance = player_distance[y][x];
    }
    ...
    if (dy < 0)
        dy = -dy;
    i32 fallback_distance = min(dx + dy, DUNGEON_PATH_UNREACHABLE - 1);
    return (i16)fallback_distance;
}

static i32 game_dungeon_find_most_threatening_enemy_in_view(
    const Game *game, const i16 player_distance[DUNGEON_ROW_COUNT][DUNGEON_COL_COUNT],
    bool player_distance_valid, i32 viewer_x, i32 viewer_y)
{
    bool has_best = false;
    ...
    for (u8 unit_idx = 0; unit_idx < game->unit_count; unit_idx++) {
        ...
        Dungeon_Threat_Source threat = {
            .x = unit->x,
            .y = unit->y,
            .distance_to_player = game_dungeon_get_player_distance_for_threat(
...
static bool game_dungeon_find_nearest_hostile_target(const Game *game, i32 unit_idx,
                                                     bool require_los,
                                                     Dungeon_Enemy_Target *out_target)
{
    ...
    i16 goal_x[1] = {(i16)actor_x};
    i16 goal_y[1] = {(i16)actor_y};
    i16 distance_from_actor[DUNGEON_ROW_COUNT][DUNGEON_COL_COUNT];
    bool has_distance_from_actor =
        game_dungeon_build_distance_field(game, goal_x, goal_y, 1, distance_from_actor);

    if (!actor_is_friendly && game_dungeon_cell_in_bounds(game->player_x, game->player_y) &&
        (!require_los ||
         game_dungeon_unit_can_see_cell(game, actor_x, actor_y, game->player_x, game->player_y))) {
        i16 player_distance = DUNGEON_PATH_UNREACHABLE;
        if (game_dungeon_cells_are_cardinal_neighbors(actor_x, actor_y, game->player_x,
                                                      game->player_y)) {
            player_distance = 1;
        } else if (has_distance_from_actor &&
                   game_dungeon_cell_in_bounds(game->player_x, game->player_y)) {
            player_distance = distance_from_actor[game->player_y][game->player_x];
        }

        if (player_distance > 0 && player_distance < DUNGEON_PATH_UNREACHABLE) {
            best_target.distance = player_distance;
            has_target = true;
        }
    }
    ...
    for (u8 candidate_idx = 0; candidate_idx < game->unit_count; candidate_idx++) {
        ...
        if (!game_dungeon_cell_is_floor(game, candidate->x, candidate->y))
            continue;
        if (require_los &&
            !game_dungeon_unit_can_see_cell(game, actor_x, actor_y, candidate->x, candidate->y)) {
            continue;
        }
        if (!has_distance_from_actor)
            continue;

        i16 distance = distance_from_actor[candidate->y][candidate->x];
        if (distance <= 0 || distance >= DUNGEON_PATH_UNREACHABLE)
            continue;
        if (has_target && distance >= best_target.distance)
            continue;
...
static bool game_dungeon_find_nearest_visible_hostile_target(const Game *game, i32 unit_idx,
                                                             Dungeon_Enemy_Target *out_target)
{
    return game_dungeon_find_nearest_hostile_target(game, unit_idx, true, out_target);
}

static bool game_dungeon_find_nearest_aligned_hostile_target(const Game *game, i32 unit_idx,
                                                             i32 max_range, bool require_los,
                                                             bool include_diagonals,
                                                             bool prefer_diagonal_targets,
                                                             Dungeon_Enemy_Target *out_target)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    ...
    bool has_target = false;
    bool best_target_is_diagonal = false;
    Dungeon_Enemy_Target best_target = {
        .target_is_player = true,
        .target_x = game->player_x,
        .target_y = game->player_y,
    ...
    if (!actor_is_friendly && game_dungeon_cell_in_bounds(game->player_x, game->player_y) &&
        game_dungeon_cells_are_aligned(actor_x, actor_y, game->player_x, game->player_y,
                                       include_diagonals) &&
        (!require_los ||
        ...
        bool within_range = distance > 0;
        if (max_range > 0)
            within_range = within_range && distance <= max_range;
        if (within_range) {
            i32 player_dx = game->player_x - actor_x;
            if (player_dx < 0)
                player_dx = -player_dx;
            i32 player_dy = game->player_y - actor_y;
            if (player_dy < 0)
                player_dy = -player_dy;

            best_target.distance = (i16)distance;
            best_target_is_diagonal = player_dx == player_dy && player_dx > 0;
            has_target = true;
        }
    }
    ...
    for (u8 candidate_idx = 0; candidate_idx < game->unit_count; candidate_idx++) {
        ...
        if (!game_dungeon_cell_in_bounds(candidate->x, candidate->y))
            continue;
        if (!game_dungeon_cells_are_aligned(actor_x, actor_y, candidate->x, candidate->y,
                                            include_diagonals)) {
            continue;
        }
        if (require_los &&
            !game_dungeon_has_line_of_sight(game, actor_x, actor_y, candidate->x, candidate->y)) {
            continue;
        }

        i32 distance = game_dungeon_get_line_distance(actor_x, actor_y, candidate->x, candidate->y);
        if (distance <= 0)
            continue;
        if (max_range > 0 && distance > max_range)
            continue;

        i32 candidate_dx = candidate->x - actor_x;
        if (candidate_dx < 0)
            candidate_dx = -candidate_dx;
        i32 candidate_dy = candidate->y - actor_y;
        if (candidate_dy < 0)
            candidate_dy = -candidate_dy;
        bool candidate_is_diagonal = candidate_dx == candidate_dy && candidate_dx > 0;

        bool is_better = false;
        if (!has_target) {
            is_better = true;
        } else if (prefer_diagonal_targets && candidate_is_diagonal != best_target_is_diagonal) {
            is_better = candidate_is_diagonal;
        } else if (distance < best_target.distance) {
            is_better = true;
        } else if (distance == best_target.distance && best_target.target_is_player) {
            is_better = true;
        } else if (distance == best_target.distance && !best_target.target_is_player &&
                   (i32)candidate_idx < best_target.target_unit_idx) {
            is_better = true;
        }
        ...
        best_target.distance = (i16)distance;
        best_target.target_unit_idx = candidate_idx;
        best_target_is_diagonal = candidate_is_diagonal;
        has_target = true;
    }
...
static bool game_dungeon_find_nearest_visible_aligned_hostile_target(
    const Game *game, i32 unit_idx, i32 max_range, bool include_diagonals,
    bool prefer_diagonal_targets, Dungeon_Enemy_Target *out_target)
{
    return game_dungeon_find_nearest_aligned_hostile_target(
        game, unit_idx, max_range, true, include_diagonals, prefer_diagonal_targets, out_target);
}
                                                    ...
                                                    i32 target_y)
{
    ...
    if (best_distance <= 0 || best_distance >= DUNGEON_PATH_UNREACHABLE)
        return false;
...
static bool game_dungeon_try_move_unit_to_player_distance(
    Game *game, i32 unit_idx, i32 desired_distance,
    const i16 player_distance[DUNGEON_ROW_COUNT][DUNGEON_COL_COUNT], bool player_distance_valid)
{
    assert(unit_idx >= 0);
    assert(unit_idx < game->unit_count);
    ...
    i16 current_player_distance = player_distance[start_y][start_x];
    if (current_player_distance < 0 || current_player_distance >= DUNGEON_PATH_UNREACHABLE)
        return false;
    ...
    for (i32 n = 0; n < 4; n++) {
        ...
        i16 next_player_distance = player_distance[ny][nx];
        if (next_player_distance < 0 || next_player_distance >= DUNGEON_PATH_UNREACHABLE)
            continue;
...
static bool game_dungeon_get_dash_destination_cell(const Game *game, i32 unit_idx, i32 target_x,
                                                   i32 target_y, i32 max_range, i32 *out_dash_x,
                                                   i32 *out_dash_y)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    ...
    if (target_distance <= 1 || target_distance > max_range)
        return false;
    ...
    while (scan_x != target_x || scan_y != target_y) {
        ...
        if (!game_dungeon_cell_is_floor(game, scan_x, scan_y))
            return false;
        if (game_dungeon_cell_has_world_feature(game, scan_x, scan_y))
            return false;
        if (game_dungeon_cell_has_item(game, scan_x, scan_y))
            return false;
...
static bool game_dungeon_find_closest_line_cell_to_target(const Game *game, i32 unit_idx,
                                                          i32 target_x, i32 target_y,
                                                          bool include_diagonals, i32 *out_x,
                                                          i32 *out_y)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    ...
    for (i32 y = 0; y < DUNGEON_ROW_COUNT; y++) {
        for (i32 x = 0; x < DUNGEON_COL_COUNT; x++) {
            if (!game_dungeon_cell_is_floor(game, x, y))
                continue;
            if (!game_dungeon_cells_are_aligned(x, y, target_x, target_y, include_diagonals))
                continue;
            ...
            i32 target_distance = game_dungeon_get_line_distance(x, y, target_x, target_y);
                                                  ...
                                                  const Dungeon_Enemy_Target *target)
{
    ...
    i32 dash_x = 0;
    i32 dash_y = 0;
    if (!game_dungeon_get_dash_destination_cell(game, unit_idx, target_x, target_y,
                                                DUNGEON_FAMILIAR_BAT_DASH_RANGE, &dash_x,
                                                &dash_y)) {
        return false;
    }
...
static bool game_dungeon_try_knockback_player(Game *game, i32 attacker_x, i32 attacker_y)
{
    assert(game != 0);

    if (!game_dungeon_cells_are_cardinal_neighbors(attacker_x, attacker_y, game->player_x,
                                                   game->player_y)) {
        return false;
    }

    i32 knock_dx = game->player_x - attacker_x;
    i32 knock_dy = game->player_y - attacker_y;
    i32 knock_x = game->player_x + knock_dx;
    i32 knock_y = game->player_y + knock_dy;
    if (!game_dungeon_cell_is_floor(game, knock_x, knock_y))
        return false;
    if (game_dungeon_cell_is_occupied(game, knock_x, knock_y))
        return false;
...
static bool game_dungeon_try_knockback_unit(Game *game, i32 target_unit_idx, i32 attacker_x,
                                            i32 attacker_y)
{
    assert(game != 0);
    assert(target_unit_idx >= 0);
    assert(target_unit_idx < game->unit_count);

    Dungeon_Unit *target_unit = &game->units[target_unit_idx];
    if (!game_dungeon_cells_are_cardinal_neighbors(attacker_x, attacker_y, target_unit->x,
                                                   target_unit->y)) {
        return false;
    }

    i32 knock_dx = target_unit->x - attacker_x;
    i32 knock_dy = target_unit->y - attacker_y;
    i32 knock_x = target_unit->x + knock_dx;
    i32 knock_y = target_unit->y + knock_dy;
    if (!game_dungeon_cell_is_floor(game, knock_x, knock_y))
        return false;
    if (game_dungeon_cell_is_occupied(game, knock_x, knock_y))
        return false;

    i32 start_x = target_unit->x;
    i32 start_y = target_unit->y;
    game_dungeon_begin_unit_move_animation(target_unit, start_x, start_y);
    target_unit->x = (i16)knock_x;
...
static bool game_dungeon_take_reaper_sweep_attack(Game *game, i32 unit_idx, i32 preferred_target_x,
                                                  i32 preferred_target_y, float attack_delay)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    assert(unit_idx < game->unit_count);
    ...
    bool unit_is_friendly = unit->is_friendly;
    bool has_anim_target = false;
    i32 anim_target_x = preferred_target_x;
    i32 anim_target_y = preferred_target_y;
    if (game_dungeon_cells_are_cardinal_neighbors(start_x, start_y, preferred_target_x,
                                                  preferred_target_y)) {
        has_anim_target = true;
    }

    bool hit_player = false;
    if (!unit_is_friendly &&
        game_dungeon_cells_are_cardinal_neighbors(start_x, start_y, game->player_x,
                                                  game->player_y) &&
        game_dungeon_unit_can_attack_cell(game, unit, game->player_x, game->player_y)) {
        hit_player = true;
        if (!has_anim_target) {
            anim_target_x = game->player_x;
            anim_target_y = game->player_y;
            attack_delay = game_dungeon_get_player_move_anim_remaining(game);
            has_anim_target = true;
        }
    }
    ...
    for (i32 idx = 0; idx < game->unit_count; idx++) {
        if (idx == unit_idx)
            continue;

        const Dungeon_Unit *candidate = &game->units[idx];
        if (candidate->is_friendly == unit_is_friendly)
            continue;
        if (!game_dungeon_cells_are_cardinal_neighbors(start_x, start_y, candidate->x,
                                                       candidate->y))
            continue;
        if (!game_dungeon_unit_can_attack_cell(game, unit, candidate->x, candidate->y))
            continue;

        if (hit_unit_count < DUNGEON_MAX_UNITS)
            hit_unit_indices[hit_unit_count++] = idx;

        if (!has_anim_target) {
            anim_target_x = candidate->x;
            anim_target_y = candidate->y;
            attack_delay = game_dungeon_get_unit_move_anim_remaining(candidate);
            has_anim_target = true;
        }
    }
    ...
    unit->orientation = game_dungeon_get_orientation_from_step(
        anim_target_x - start_x, anim_target_y - start_y, unit->orientation);
    game_dungeon_begin_unit_attack_animation(unit, start_x, start_y, anim_target_x, anim_target_y,
                                             attack_delay);
    ...
    for (i32 i = 0; i < hit_unit_count - 1; i++) {
        for (i32 j = i + 1; j < hit_unit_count; j++) {
            if (hit_unit_indices[j] > hit_unit_indices[i]) {
                i32 swap = hit_unit_indices[i];
                hit_unit_indices[i] = hit_unit_indices[j];
                hit_unit_indices[j] = swap;
            }
        }
    }

    for (i32 hit_idx = 0; hit_idx < hit_unit_count; hit_idx++) {
        i32 target_idx = hit_unit_indices[hit_idx];
        if (target_idx < 0 || target_idx >= game->unit_count)
            continue;

        Dungeon_Unit *target_unit = &game->units[target_idx];
        if (target_unit->is_friendly == unit_is_friendly)
            continue;
        if (!game_dungeon_cells_are_cardinal_neighbors(start_x, start_y, target_unit->x,
                                                       target_unit->y)) {
            continue;
        }
        if (!game_dungeon_unit_can_attack_cell(game, unit, target_unit->x, target_unit->y))
            continue;

        if (unit_is_friendly && !target_unit->is_friendly) {
            target_unit->is_awake = true;
            target_unit->turns_out_of_player_los = 0;
        }
                                               ...
                                               const Dungeon_Enemy_Target *target)
{
    ...
    if (target->target_is_player) {
        ...
        if (target_unit->x != target->target_x || target_unit->y != target->target_y ||
            target_unit->is_friendly == unit->is_friendly) {
            return;
        }
    ...
    if (unit->kind == UNIT_ART_REAPER) {
        if (game_dungeon_take_reaper_sweep_attack(game, unit_idx, target_x, target_y, attack_delay))
            return;
    }
...
static void game_dungeon_take_basic_ranged_turn(Game *game, i32 unit_idx,
                                                const Dungeon_Enemy_Target *target, i32 min_range,
                                                i32 max_range)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    ...
    i32 target_distance = target->distance;
    bool in_attack_range = target_distance >= min_range && target_distance <= max_range;
    if (in_attack_range &&
        ...
        if (unit->kind == UNIT_ART_IMP) {
            u16 target_unit_id = target->target_is_player ? 0 : target_unit->id;
            game_dungeon_spawn_web_projectile(game, start_x, start_y, target_x, target_y,
                                              target->target_is_player, target_unit_id,
...
static bool game_dungeon_take_aligned_piercing_ranged_attack(Game *game, i32 unit_idx,
                                                             const Dungeon_Enemy_Target *target,
                                                             i32 range)
{
    assert(game != 0);
    ...
    for (i32 path_idx = 0; path_idx < path_count; path_idx++) {
        ...
        i32 hit_unit_idx = game_dungeon_get_unit_index_at(game, scan_x, scan_y);
        if (hit_unit_idx < 0 || hit_unit_idx == unit_idx)
            continue;
...
static bool game_dungeon_try_move_aligned_ranged_unit_to_firing_position(
    Game *game, i32 unit_idx, i32 target_x, i32 target_y, i32 range, i32 min_target_distance,
    i32 min_enemy_distance, bool prefer_diagonal_alignment)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    assert(unit_idx < game->unit_count);
    assert(range > 0);
    assert(min_target_distance > 0);
    assert(min_enemy_distance >= 0);
    ...
    i32 preferred_target_distance = min(range, max(min_target_distance + 2, 3));

    bool has_best = false;
    i32 best_x = start_x;
    i32 best_y = start_y;
    i16 best_unit_distance = DUNGEON_PATH_UNREACHABLE;
    i16 best_nearest_enemy_distance = 0;
    i32 best_enemy_error = 0;
    i32 best_target_error = 0;
    i32 best_target_center_error = 0;
    i32 best_target_distance = 0;
    ...
    for (i32 y = 0; y < DUNGEON_ROW_COUNT; y++) {
        for (i32 x = 0; x < DUNGEON_COL_COUNT; x++) {
            ...
            i16 unit_distance = distance_from_unit[y][x];
            if (unit_distance < 0 || unit_distance >= DUNGEON_PATH_UNREACHABLE)
                continue;
            ...
            i16 nearest_enemy_distance = DUNGEON_PATH_UNREACHABLE;
            if (has_enemy_distance) {
                nearest_enemy_distance = distance_to_nearest_enemy[y][x];
                if (nearest_enemy_distance < 0 ||
                    nearest_enemy_distance >= DUNGEON_PATH_UNREACHABLE)
                    continue;
            }
            ...
            if (target_distance < min_target_distance)
                target_error = min_target_distance - target_distance;

            i32 target_center_error = target_distance - preferred_target_distance;
            if (target_center_error < 0)
                target_center_error = -target_center_error;
            ...
            if (!has_best) {
                ...
                is_better = nearest_enemy_distance > best_nearest_enemy_distance;
            } else if (target_error != best_target_error) {
                is_better = target_error < best_target_error;
            } else if (prefer_diagonal_alignment && is_diagonal != best_is_diagonal) {
                is_better = is_diagonal;
            } else if (target_center_error != best_target_center_error) {
                is_better = target_center_error < best_target_center_error;
            } else if (unit_distance != best_unit_distance) {
                is_better = unit_distance < best_unit_distance;
            } else if (target_distance != best_target_distance) {
                is_better = target_distance > best_target_distance;
            } else if (y < best_y || (y == best_y && x < best_x)) {
                is_better = true;
            }
    ...
    return game_dungeon_try_move_unit_towards_cell(game, unit_idx, best_x, best_y);
}

static void game_dungeon_take_aligned_ranged_turn(Game *game, i32 unit_idx, i32 range,
                                                  i32 too_close_distance)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    assert(unit_idx < game->unit_count);
    assert(range > 0);
    assert(too_close_distance > 0);

    Dungeon_Unit *unit = &game->units[unit_idx];
    bool prefer_visible_targets = unit->is_friendly;
    ...
    if (game_dungeon_try_move_aligned_ranged_unit_to_firing_position(
            game, unit_idx, target_x, target_y, range, desired_min_distance, min_enemy_distance,
            true)) {
        return;
    }
                                                     ...
                                                     float start_delay)
{
    ...
    game_dungeon_spawn_beetle_explosion_fx(game, center_x, center_y, start_delay);

    if (game_dungeon_cell_is_in_kamikaze_area(center_x, center_y, game->player_x, game->player_y)) {
...
static void game_dungeon_trigger_kamikaze_explosion(Game *game, i32 unit_idx)
{
    ...
    float explosion_delay = game_dungeon_get_unit_move_anim_remaining(&exploding_unit);

    game_dungeon_remove_unit_at(game, unit_idx);

    game_dungeon_trigger_beetle_explosion_at(game, &exploding_unit, explosion_delay);
}
                                            ...
                                            const Dungeon_Enemy_Target *target)
{
    ...
    if (game_dungeon_cells_are_cardinal_neighbors(start_x, start_y, target_x, target_y) &&
        game_dungeon_unit_can_attack_cell(game, unit, target_x, target_y)) {
        game_dungeon_begin_unit_attack_animation(unit, start_x, start_y, target_x, target_y,
                                                 attack_delay);
...
static void game_dungeon_take_spider_turn(Game *game, i32 unit_idx)
{
    ...
    if (in_web_range &&
        ...
        u16 target_unit_id = target.target_is_player ? 0 : target_unit->id;
        game_dungeon_spawn_web_projectile(game, start_x, start_y, target_x, target_y,
                                          target.target_is_player, target_unit_id, attack_delay,
...
static bool game_dungeon_take_bat_dash_behavior_toward_target(Game *game, i32 unit_idx,
                                                              const Dungeon_Enemy_Target *target)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    ...
    bool aligned_at_turn_start =
        game_dungeon_cells_are_line_aligned(start_x, start_y, target_x, target_y);
    ...
    if (aligned_at_turn_start && game_dungeon_try_take_bat_dash_attack(game, unit_idx, target))
        return true;

    if (aligned_at_turn_start) {
        return game_dungeon_try_move_unit_towards_cell_with_max_steps(
            game, unit_idx, target_x, target_y, DUNGEON_FAMILIAR_BAT_MAX_MOVE_TILES);
    }
...
static void game_dungeon_take_enemy_dash_turn(Game *game, i32 unit_idx)
{
    ...
    Dungeon_Enemy_Target target = {0};
    bool has_target = game_dungeon_find_nearest_aligned_hostile_target(game, unit_idx, -1, false,
                                                                       false, false, &target);
    if (!has_target)
        has_target = game_dungeon_find_nearest_hostile_target(game, unit_idx, false, &target);
    if (!has_target)
        return;

    game_dungeon_take_bat_dash_behavior_toward_target(game, unit_idx, &target);
}

static void game_dungeon_take_dash_through_turn(Game *game, i32 unit_idx,
                                                const Dungeon_Enemy_Target *target, i32 dash_range,
                                                i32 cooldown_turns)
{
    assert(game != 0);
    assert(unit_idx >= 0);
    ...
    bool attacker_is_friendly = unit->is_friendly;
    bool hit_player = false;
    i32 hit_unit_indices[DUNGEON_MAX_UNITS];
    i32 hit_unit_count = 0;
    for (i32 path_idx = 0; path_idx < dash_steps_taken; path_idx++) {
        ...
        if (hit_unit_idx < 0 || hit_unit_idx == unit_idx)
            continue;
        if (game->units[hit_unit_idx].is_friendly == attacker_is_friendly)
            continue;
    ...
    i32 destination_unit_idx = game_dungeon_get_unit_index_at(game, dash_end_x, dash_end_y);
    if (destination_unit_idx >= 0 && destination_unit_idx != unit_idx) {
        i32 displaced_x = 0;
        i32 displaced_y = 0;
        if (!game_dungeon_find_nearest_unoccupied_floor_cell(game, dash_end_x, dash_end_y, 0,
                                                             &displaced_x, &displaced_y)) {
            return;
        }

        Dungeon_Unit *displaced = &game->units[destination_unit_idx];
        i32 displaced_start_x = displaced->x;
        i32 displaced_start_y = displaced->y;
        game_dungeon_begin_unit_move_animation(displaced, displaced_start_x, displaced_start_y);
        displaced->x = (i16)displaced_x;
        displaced->y = (i16)displaced_y;
        displaced->orientation = game_dungeon_get_orientation_from_step(
            displaced_x - displaced_start_x, displaced_y - displaced_start_y,
            displaced->orientation);
    }

    if (game->player_x == dash_end_x && game->player_y == dash_end_y) {
        i32 displaced_x = 0;
        i32 displaced_y = 0;
        if (!game_dungeon_find_nearest_unoccupied_floor_cell(game, dash_end_x, dash_end_y, 0,
                                                             &displaced_x, &displaced_y)) {
        ...
        i32 player_start_x = game->player_x;
        i32 player_start_y = game->player_y;
        game_dungeon_begin_player_move_animation(game, player_start_x, player_start_y);
        game->player_x = (i16)displaced_x;
        game->player_y = (i16)displaced_y;
        game_dungeon_rebuild_line_of_sight(game);
    }

    game_dungeon_begin_unit_move_animation(unit, start_x, start_y);
    unit->x = (i16)dash_end_x;
    unit->y = (i16)dash_end_y;
    unit->orientation = game_dungeon_get_orientation_from_positions(
        dash_end_x, dash_end_y, target_x, target_y, unit->orientation);
    ...
    for (i32 hit_idx = 0; hit_idx < hit_unit_count; hit_idx++) {
        ...
        Dungeon_Unit *target_unit = &game->units[target_idx];
        if (target_unit->is_friendly == attacker_is_friendly)
            continue;

        if (attacker_is_friendly && !target_unit->is_friendly) {
            target_unit->is_awake = true;
            target_unit->turns_out_of_player_los = 0;
        }
    ...
    bool player_distance_valid)
{
    ...
    if ((ai_kind == UNIT_AI_DASH || ai_kind == UNIT_AI_DASH_THROUGH) &&
    ...
    case UNIT_AI_ALIGNED_RANGED:
        follow_min_distance = DUNGEON_FAMILIAR_BASIC_RANGED_FOLLOW_MIN_DISTANCE;
        follow_max_distance = DUNGEON_FAMILIAR_BASIC_RANGED_FOLLOW_MAX_DISTANCE;
        break;
    ...
    if (unit->is_returning_to_player) {
        ...
        if (unit_idx >= game->unit_count)
            return;

        Dungeon_Unit *current_unit = &game->units[unit_idx];
        if (!current_unit->is_friendly)
            return;
    ...
    case UNIT_AI_BASIC_RANGED: {
        ...
        if (game_dungeon_find_nearest_visible_hostile_target(game, unit_idx, &target)) {
            game_dungeon_take_basic_ranged_turn(game, unit_idx, &target,
                                                DUNGEON_FAMILIAR_BASIC_RANGED_ATTACK_MIN_DISTANCE,
                                                DUNGEON_FAMILIAR_BASIC_RANGED_ATTACK_MAX_DISTANCE);
            return;
        }
    ...
    case UNIT_AI_ALIGNED_RANGED: {
        Dungeon_Enemy_Target target = {0};
        if (game_dungeon_find_nearest_visible_hostile_target(game, unit_idx, &target)) {
            game_dungeon_take_aligned_ranged_turn(game, unit_idx,
                                                  DUNGEON_BEHOLDER_ALIGNED_RANGED_RANGE,
                                                  DUNGEON_ALIGNED_RANGED_TOO_CLOSE_DISTANCE);
            return;
        }
    ...
    case UNIT_AI_DEFEND_RANGED: {
        ...
        if (target_enemy_idx >= 0) {
            ...
            if (!target_enemy->is_webbed && no_enemy_too_close && in_web_range) {
                ...
                game_dungeon_begin_unit_attack_animation(unit, start_x, start_y, enemy_x, enemy_y,
                                                         attack_delay);
                game_dungeon_spawn_web_projectile(game, start_x, start_y, enemy_x, enemy_y, false,
                                                  target_enemy->id, attack_delay,
                                                  (Color){244, 247, 255, 232});
    ...
    case UNIT_AI_DASH: {
        Dungeon_Enemy_Target target = {0};
        bool has_target = game_dungeon_find_nearest_visible_aligned_hostile_target(
            game, unit_idx, -1, false, false, &target);
        if (!has_target)
            has_target = game_dungeon_find_nearest_visible_hostile_target(game, unit_idx, &target);
        if (has_target) {
            game_dungeon_take_bat_dash_behavior_toward_target(game, unit_idx, &target);
            return;
        }
    ...
    case UNIT_AI_DASH_THROUGH: {
        Dungeon_Enemy_Target target = {0};
        bool has_target = game_dungeon_find_nearest_visible_hostile_target(game, unit_idx, &target);
        ...
        if (has_target) {
            game_dungeon_take_dash_through_turn(game, unit_idx, &target,
                                                DUNGEON_GRIFFON_DASH_THROUGH_RANGE,
                                                DUNGEON_GRIFFON_DASH_THROUGH_COOLDOWN_TURNS);
            return;
        }
...
static void game_dungeon_take_friendly_turns(Game *game)
{
    ...
    i16 player_distance[DUNGEON_ROW_COUNT][DUNGEON_COL_COUNT];
    bool player_distance_valid = game_dungeon_build_player_distance_field(game, player_distance);
...
static void game_dungeon_take_enemy_turns(Game *game)
{
    ...
    while (true) {
        ...
        game->units[next_enemy_idx].has_acted_this_turn = true;

        Dungeon_Unit *unit = &game->units[next_enemy_idx];
        i32 start_x = unit->x;
        i32 start_y = unit->y;
        if (!game_dungeon_cell_in_bounds(start_x, start_y))
            continue;
        ...
        if (!unit->is_awake) {
            if (!game_dungeon_cell_is_in_player_los(game, start_x, start_y))
                continue;
        ...
        case UNIT_AI_TURRET:
            ...
            break;
        case UNIT_AI_ALIGNED_RANGED:
            game_dungeon_take_aligned_ranged_turn(game, next_enemy_idx,
                                                  DUNGEON_BEHOLDER_ALIGNED_RANGED_RANGE,
                                                  DUNGEON_ALIGNED_RANGED_TOO_CLOSE_DISTANCE);
            break;
        case UNIT_AI_BASIC_RANGED: {
            Dungeon_Enemy_Target target = {0};
            if (game_dungeon_find_nearest_hostile_target(game, next_enemy_idx, false, &target)) {
                game_dungeon_take_basic_ranged_turn(
                    game, next_enemy_idx, &target,
                    DUNGEON_FAMILIAR_BASIC_RANGED_ATTACK_MIN_DISTANCE,
                    DUNGEON_FAMILIAR_BASIC_RANGED_ATTACK_MAX_DISTANCE);
            }
            break;
        ...
        case UNIT_AI_DEFEND_RANGED:
            ...
            {
                Dungeon_Enemy_Target target = {0};
                if (game_dungeon_find_nearest_hostile_target(game, next_enemy_idx, false,
                                                             &target)) {
        ...
        case UNIT_AI_DASH_THROUGH: {
            ...
            if (game_dungeon_find_nearest_hostile_target(game, next_enemy_idx, false, &target)) {
                if (unit->dash_cooldown_turns > 0) {
                    ...
                    game_dungeon_take_dash_through_turn(
                        game, next_enemy_idx, &target, DUNGEON_GRIFFON_DASH_THROUGH_RANGE,
                        DUNGEON_GRIFFON_DASH_THROUGH_COOLDOWN_TURNS);
                }
            }
            break;
...
static UNIT_ART_KIND game_dungeon_pick_shaman_companion_kind_for_depth(RNG *rng, u32 depth)
{
    ...
    u32 depth_idx = game_dungeon_clamp_run_depth(depth);
    i32 companion_kind_idx = game_pick_weighted_index(rng, goblin_companion_weights[depth_idx], 2);
...
static bool game_dungeon_enemy_spawn_cell_is_valid(const Game *game, i32 x, i32 y, i32 stairs_up_x,
                                                   i32 stairs_up_y)
{
    if (!game_dungeon_cell_is_floor(game, x, y))
        return false;
                                                     ...
                                                     i32 stairs_up_y, i16 *out_x, i16 *out_y)
{
    for (i32 attempt = 0; attempt < 512; attempt++) {
        i32 x = ck_rand_int(rng, 0, DUNGEON_COL_COUNT);
        i32 y = ck_rand_int(rng, 0, DUNGEON_ROW_COUNT);
        if (!game_dungeon_enemy_spawn_cell_is_valid(game, x, y, stairs_up_x, stairs_up_y))
            continue;
                                                              ...
                                                              i16 *out_y)
{
    ...
    i16 chosen_x = -1;
    i16 chosen_y = -1;
    for (i32 offset_idx = 0; offset_idx < 8; offset_idx++) {
        i32 x = center_x + neighbor_offsets[offset_idx][0];
        i32 y = center_y + neighbor_offsets[offset_idx][1];
        if (!game_dungeon_enemy_spawn_cell_is_valid(game, x, y, stairs_up_x, stairs_up_y))
            continue;
                                              ...
                                              bool include_up_stairs)
{
    ...
    memset(game->tile_fx, 0, sizeof(game->tile_fx));
    game->tile_fx_cursor = 0;
    ...
    game_dungeon_rebuild_line_of_sight(game);
    i16 player_distance[DUNGEON_ROW_COUNT][DUNGEON_COL_COUNT];
    return game_dungeon_build_player_distance_field(game, player_distance);
}
                                        ...
                                        u32 buffer_cap)
{
    ...
    if (row >= 0 && row <= 3 && entry.col_start == DUNGEON_BEETLE_EXPLOSION_FX_COL_START &&
        entry.frame_count == DUNGEON_BEETLE_EXPLOSION_FX_FRAME_COUNT) {
        snprintf(buffer, buffer_cap, "explosion%d", row + 1);
        return;
    }
...
static void game_dungeon_apply_unit_attack_dash(const Dungeon_Unit *unit, float *io_tile_x,
                                                float *io_tile_y)
{
    assert(unit != 0);
    assert(io_tile_x != 0);
    ...
    if (!unit->attack_anim_active)
        return;
    if (unit->attack_anim_delay > 0.0f)
        return;

    float attack_t = game_dungeon_get_attack_anim_t(unit->attack_anim_elapsed);
    float dash_amount = game_dungeon_get_attack_anim_dash_amount(attack_t);
    *io_tile_x += unit->attack_anim_dir_x * dash_amount * DUNGEON_ATTACK_DASH_DISTANCE_TILES;
    *io_tile_y += unit->attack_anim_dir_y * dash_amount * DUNGEON_ATTACK_DASH_DISTANCE_TILES;
}
                                                        ...
                                                        float tile_size)
{
    ...
    game_dungeon_apply_unit_attack_dash(unit, &tile_x, &tile_y);
...
static Vector2
game_dungeon_get_defeated_unit_visual_feet_position(const Dungeon_Defeated_Unit_Visual *visual,
                                                    Vector2 origin, float tile_size)
{
    assert(visual != 0);

    float tile_x = (float)visual->x;
    float tile_y = (float)visual->y;
    float hop = 0.0f;

    if (visual->move_anim_active) {
        float t = game_dungeon_get_move_anim_t(visual->move_anim_elapsed);
        game_dungeon_get_move_anim_tile_position(visual->move_anim_from_x, visual->move_anim_from_y,
                                                 (float)visual->x, (float)visual->y,
                                                 visual->move_anim_elapsed, &tile_x, &tile_y);
        hop = 4.0f * t * (1.0f - t);
    }
...
static void game_dungeon_apply_player_attack_dash(const Game *game, float *io_tile_x,
                                                  float *io_tile_y)
{
    assert(game != 0);
    assert(io_tile_x != 0);
    ...
    if (!game->player_attack_anim_active)
        return;
    if (game->player_attack_anim_delay > 0.0f)
        return;

    float attack_t = game_dungeon_get_attack_anim_t(game->player_attack_anim_elapsed);
    float dash_amount = game_dungeon_get_attack_anim_dash_amount(attack_t);
    *io_tile_x += game->player_attack_anim_dir_x * dash_amount * DUNGEON_ATTACK_DASH_DISTANCE_TILES;
    *io_tile_y += game->player_attack_anim_dir_y * dash_amount * DUNGEON_ATTACK_DASH_DISTANCE_TILES;
}
                                                          ...
                                                          float tile_size)
{
    ...
    game_dungeon_apply_player_attack_dash(game, &tile_x, &tile_y);
...
static void game_dungeon_update_unit_animations(Game *game)
{
    float frame_time = GetFrameTime();
    if (frame_time <= 0.0f)
        frame_time = CAMERA_FALLBACK_FRAME_TIME;
    ...
    if (game->player_attack_anim_active) {
        float attack_frame_time = frame_time;
        if (game->player_attack_anim_delay > 0.0f) {
            game->player_attack_anim_delay -= frame_time;
            if (game->player_attack_anim_delay > 0.0f) {
                attack_frame_time = 0.0f;
            } else {
                attack_frame_time = -game->player_attack_anim_delay;
                game->player_attack_anim_delay = 0.0f;
            }
        }

        game->player_attack_anim_elapsed += attack_frame_time;
        if (game->player_attack_anim_elapsed >= DUNGEON_ATTACK_ANIM_DURATION) {
            game->player_attack_anim_elapsed = DUNGEON_ATTACK_ANIM_DURATION;
            game->player_attack_anim_dir_x = 0.0f;
            game->player_attack_anim_dir_y = 0.0f;
            game->player_attack_anim_delay = 0.0f;
    ...
    for (u8 unit_idx = 0; unit_idx < game->unit_count; unit_idx++) {
        ...
        if (unit->attack_anim_active) {
            float attack_frame_time = frame_time;
            if (unit->attack_anim_delay > 0.0f) {
                unit->attack_anim_delay -= frame_time;
                if (unit->attack_anim_delay > 0.0f) {
                    attack_frame_time = 0.0f;
                } else {
                    attack_frame_time = -unit->attack_anim_delay;
                    unit->attack_anim_delay = 0.0f;
                }
            }

            unit->attack_anim_elapsed += attack_frame_time;
            if (unit->attack_anim_elapsed >= DUNGEON_ATTACK_ANIM_DURATION) {
                unit->attack_anim_elapsed = DUNGEON_ATTACK_ANIM_DURATION;
                unit->attack_anim_dir_x = 0.0f;
                unit->attack_anim_dir_y = 0.0f;
                unit->attack_anim_delay = 0.0f;
    ...
    for (u8 visual_idx = 0; visual_idx < DUNGEON_MAX_UNITS; visual_idx++) {
        Dungeon_Defeated_Unit_Visual *visual = &game->defeated_unit_visuals[visual_idx];
        if (!visual->move_anim_active)
            continue;

        visual->move_anim_elapsed += frame_time;
        if (visual->move_anim_elapsed >= DUNGEON_MOVE_ANIM_DURATION) {
            visual->move_anim_elapsed = DUNGEON_MOVE_ANIM_DURATION;
            visual->move_anim_active = false;
        }
    }
}
...
static void game_dungeon_update_tile_fx(Game *game)
{
    float frame_time = GetFrameTime();
    if (frame_time <= 0.0f)
        frame_time = CAMERA_FALLBACK_FRAME_TIME;
    ...
    for (i32 fx_idx = 0; fx_idx < DUNGEON_TILE_FX_CAPACITY; fx_idx++) {
        Dungeon_Tile_Fx *fx = &game->tile_fx[fx_idx];
        if (!fx->active)
            continue;

        float fx_frame_time = frame_time;
        if (fx->delay > 0.0f) {
            fx->delay -= frame_time;
            if (fx->delay > 0.0f) {
                fx_frame_time = 0.0f;
            } else {
                fx_frame_time = -fx->delay;
                fx->delay = 0.0f;
            }
        }

        fx->elapsed += fx_frame_time;
        i32 anim_frame = (i32)(fx->elapsed * fx->fps);
        if (anim_frame >= fx->frame_count)
            fx->active = false;
    }
}
                                              ...
                                              Color color)
{
    ...
    game->diseased_particle_cursor =
        (u16)((game->diseased_particle_cursor + 1) % DUNGEON_DISEASED_PARTICLE_CAPACITY);

    float half_sprite_w = tile_size * 0.4f;
    float spawn_x = ck_rand_float(&game->dungeon_diseased_particle_rng,
                                  sprite_center.x - half_sprite_w, sprite_center.x + half_sprite_w);
    float drift_x = ck_rand_float(&game->dungeon_diseased_particle_rng, -DUNGEON_TILE_SCALE * 1.5f,
                                  DUNGEON_TILE_SCALE * 1.5f);
    float rise_speed = ck_rand_float(&game->dungeon_diseased_particle_rng,
                                     -DUNGEON_TILE_SCALE * 5.0f, -DUNGEON_TILE_SCALE * 3.5f);
    float lifetime = ck_rand_float(&game->dungeon_diseased_particle_rng, 1.0f, 1.25f);

    game->diseased_particles[particle_idx] = (Dungeon_Diseased_Particle){
        .position = (Vector2){spawn_x, sprite_center.y},
...
static void game_dungeon_update_status_particles(Game *game)
{
    ...
    for (i32 idx = 0; idx < DUNGEON_DISEASED_PARTICLE_CAPACITY; idx++) {
        Dungeon_Diseased_Particle *particle = &game->diseased_particles[idx];
        if (!particle->active)
            continue;
        ...
        particle->position.x += particle->velocity.x * frame_time;
    ...
    float tile_size = WORLD_ART_TILE_SIZE * DUNGEON_TILE_SCALE;
    Vector2 dungeon_origin = game_dungeon_get_origin();
...
static void game_draw_status_particles(const Game *game)
{
    ...
    for (i32 idx = 0; idx < DUNGEON_DISEASED_PARTICLE_CAPACITY; idx++) {
        const Dungeon_Diseased_Particle *particle = &game->diseased_particles[idx];
        if (!particle->active)
            continue;

        float life_t = particle->age / max(particle->lifetime, 0.001f);
        life_t = clamp(life_t, 0.0f, 1.0f);
        u8 alpha = (u8)((1.0f - life_t) * 220.0f);
        if (alpha == 0)
            continue;
...
static void game_draw_tile_fx(const Game *game, Vector2 origin, float tile_size)
{
    float pixel_size = max(1.0f, DUNGEON_TILE_SCALE);
    float sprite_size = tile_size;

    for (i32 fx_idx = 0; fx_idx < DUNGEON_TILE_FX_CAPACITY; fx_idx++) {
        const Dungeon_Tile_Fx *fx = &game->tile_fx[fx_idx];
        if (!fx->active)
            continue;
        if (fx->delay > 0.0f)
            continue;
        ...
        i32 anim_frame = (i32)(fx->elapsed * fx->fps);
        if (anim_frame < 0 || anim_frame >= fx->frame_count)
            continue;

        Vector2 center = game_dungeon_get_cell_center(origin, fx->x, fx->y, tile_size);
        Vector2 top_left = {
        ...
        game_draw_fx_tile(game, fx->col_start + anim_frame, fx->row, top_left, sprite_size);
    }
}
...
static bool
game_dungeon_get_web_projectile_target_tile_position(const Game *game,
                                                     const Dungeon_Web_Projectile *projectile,
                                                     float *out_tile_x, float *out_tile_y)
{
    assert(game != 0);
    assert(projectile != 0);
    assert(out_tile_x != 0);
    ...
    if (projectile->target_is_player) {
        float hop = 0.0f;
        game_dungeon_get_player_draw_tile_position(game, out_tile_x, out_tile_y, &hop);
        game_dungeon_apply_player_attack_dash(game, out_tile_x, out_tile_y);
        return true;
    }

    i32 unit_idx = game_dungeon_get_unit_index_by_id(game, projectile->target_unit_id);
    if (unit_idx < 0)
        return false;

    const Dungeon_Unit *unit = &game->units[unit_idx];
    float hop = 0.0f;
    game_dungeon_get_unit_draw_tile_position(unit, out_tile_x, out_tile_y, &hop);
    game_dungeon_apply_unit_attack_dash(unit, out_tile_x, out_tile_y);
    return true;
}
...
static void game_draw_web_projectiles(const Game *game, Vector2 origin, float tile_size)
{
    ...
    for (i32 projectile_idx = 0; projectile_idx < DUNGEON_WEB_PROJECTILE_CAPACITY;
        ...
        float target_tile_x = projectile->target_x;
        float target_tile_y = projectile->target_y;
        game_dungeon_get_web_projectile_target_tile_position(game, projectile, &target_tile_x,
                                                             &target_tile_y);

        float tile_x = projectile->start_x + (target_tile_x - projectile->start_x) * eased_t;
        float tile_y = projectile->start_y + (target_tile_y - projectile->start_y) * eased_t;
...
static Color game_ui_gray_outer_color(void)
{
    return (Color){62, 62, 62, 255};
}

static Color game_ui_gray_inner_color(void)
{
...
{
    Color outer_fill = game_ui_gray_outer_color();
    Color inner_fill = game_ui_gray_inner_color();
    if (hovered) {
...
static bool game_player_try_activate_action_bar_slot_from_click(Game *game)
{
    if (game->debug_in_testing_area)
        return false;
    if (!game->input.pressed[INPUT_MOUSE_LEFT])
    ...
    if (hovered_slot >= 0)
        game_player_activate_action_bar_slot(game, hovered_slot);
    return true;
}
...
static bool game_familiar_command_button_is_enabled(const Game *game, i32 command_idx)
{
    assert(game != 0);
    assert(command_idx >= 0);
    assert(command_idx < DUNGEON_FAMILIAR_COMMAND_COUNT);

    const Dungeon_Familiar_Command_Def *def = &game_dungeon_familiar_command_defs[command_idx];
    if (!def->requires_living_familiar)
        return true;
    return game_dungeon_has_living_familiar(game);
}

static Rectangle game_familiar_command_button_rect(const Game *game, i32 command_idx)
{
    assert(game != 0);
    assert(command_idx >= 0);
    assert(command_idx < DUNGEON_FAMILIAR_COMMAND_COUNT);
...
static i32 game_familiar_command_hovered_button(const Game *game)
{
    assert(game != 0);
    ...
    for (i32 command_idx = 0; command_idx < DUNGEON_FAMILIAR_COMMAND_COUNT; command_idx++) {
        Rectangle button = game_familiar_command_button_rect(game, command_idx);
        if (game_point_in_rect(mouse, button))
            return command_idx;
    }
...
static FAMILIAR_TURN_COMMAND game_player_get_familiar_command_from_click(const Game *game,
                                                                         bool *out_consumed_click)
{
    assert(game != 0);
    assert(out_consumed_click != 0);

    *out_consumed_click = false;

    if (!game->input.pressed[INPUT_MOUSE_LEFT])
        return FAMILIAR_TURN_COMMAND_NONE;
    ...
    *out_consumed_click = true;

    i32 hovered_button = game_familiar_command_hovered_button(game);
    if (hovered_button < 0)
        return FAMILIAR_TURN_COMMAND_NONE;
    if (!game_familiar_command_button_is_enabled(game, hovered_button))
        return FAMILIAR_TURN_COMMAND_NONE;

    return game_dungeon_familiar_command_defs[hovered_button].command;
}
...
static void game_draw_familiar_command_bar(Game *game)
{
    ...
    for (i32 command_idx = 0; command_idx < DUNGEON_FAMILIAR_COMMAND_COUNT; command_idx++) {
        Rectangle button = game_familiar_command_button_rect(game, command_idx);
        const Dungeon_Familiar_Command_Def *def = &game_dungeon_familiar_command_defs[command_idx];
        bool enabled = game_familiar_command_button_is_enabled(game, command_idx);
        bool active = game->familiar_turn_command == def->command;
        bool hovered = hovered_button == command_idx;

        Color fill = game_ui_gray_inner_color();
        Color text = (Color){236, 236, 236, 255};

        if (!enabled) {
            fill = game_ui_gray_outer_color();
            text = (Color){168, 168, 168, 255};
        } else if (active || hovered) {
            fill = game_ui_hover_color(game_ui_gray_inner_color());
        } else {
        ...
        Vector2 text_measure = MeasureTextEx(game->font, def->label, text_size, game->font_spacing);
        Vector2 text_pos = {
            .x = button.x + (button.width - text_measure.x) * 0.5f,
            .y = button.y + (button.height - text_measure.y) * 0.5f,
        };
        DrawTextEx(game->font, def->label, text_pos, text_size, game->font_spacing, text);
    }
}
...
static void game_draw_player_action_bar(Game *game)
{
    ...
    for (i32 slot_idx = 0; slot_idx < DUNGEON_ACTION_BAR_SLOT_COUNT; slot_idx++) {
        ...
        bool is_active_slot = game_player_has_active_summon_target(game) &&
                              game->player_active_action_bar_slot == slot_idx;
        Color slot_fill = has_familiar ? game_ui_gray_inner_color() : game_ui_gray_outer_color();
        if (is_active_slot) {
            slot_fill = game_ui_hover_color(game_ui_gray_inner_color());
        }
        if (action_bar_disabled) {
            slot_fill = game_ui_gray_outer_color();
        }
                                       ...
                                       bool is_friendly_row, Rectangle panel)
{
    ...
    for (i32 slot_idx = 0; slot_idx < unit_kind_count; slot_idx++) {
        ...
        Color slot_fill = game_ui_gray_inner_color();
        if (selected) {
            slot_fill = game_ui_hover_color(game_ui_gray_inner_color());
        }
        if (hovered)
...
static Rectangle game_draw_floor_info_panel(Game *game)
{
    ...
    Rectangle panel = {
        ...
        .width = floor_measure.x + DUNGEON_PLAYER_PANEL_PADDING * 2.0f,
        .height = floor_size + DUNGEON_PLAYER_PANEL_PADDING * 2.0f,
    };
    ...
    DrawTextEx(game->font, floor_line, floor_pos, floor_size, game->font_spacing,
...
static Rectangle game_draw_player_stats_panel(Game *game, Rectangle floor_panel)
{
    ...
    Vector2 damage_measure =
        MeasureTextEx(game->font, damage_line, damage_size, game->font_spacing);
    float content_w = max(damage_measure.x, hearts_w);
    Vector2 status_measure = {0.0f, 0.0f};
    if (has_status) {
        status_measure = MeasureTextEx(game->font, status_line, status_size, game->font_spacing);
        content_w = max(content_w, status_measure.x);
    }

    float panel_w = content_w + (DUNGEON_PLAYER_PANEL_PADDING * 2.0f);
    float panel_h = (DUNGEON_PLAYER_PANEL_PADDING * 2.0f) + DUNGEON_PLAYER_PANEL_HEART_SIZE + 7.0f +
                    damage_size;
    if (has_status)
        panel_h += 5.0f + status_size;

    Rectangle panel = {
        .x = DUNGEON_PLAYER_PANEL_MARGIN,
        .y = floor_panel.y + floor_panel.height + DUNGEON_PLAYER_PANEL_GAP,
        .width = panel_w,
        .height = panel_h,
    };
    ...
    Vector2 damage_pos = {
        .x = panel.x + (panel.width - damage_measure.x) * 0.5f,
        .y = hearts_pos.y + DUNGEON_PLAYER_PANEL_HEART_SIZE + 7.0f,
    };
    DrawTextEx(game->font, damage_line, damage_pos, damage_size, game->font_spacing,
               (Color){234, 234, 234, 255});

    if (has_status) {
        Vector2 status_pos = {
            .x = panel.x + (panel.width - status_measure.x) * 0.5f,
            .y = damage_pos.y + damage_size + 5.0f,
        };
        DrawTextEx(game->font, status_line, status_pos, status_size, game->font_spacing,
...
static bool game_mouse_is_over_dungeon_hud_panel(Game *game, Vector2 mouse_screen,
                                                 Rectangle floor_panel, Rectangle player_panel,
                                                 Rectangle minimap_panel, Rectangle familiar_panel,
                                                 Rectangle action_bar_panel)
{
    if (game_point_in_rect(mouse_screen, floor_panel))
        return true;
...
static void game_fill_hovered_unit_from_kind(Dungeon_Hovered_Unit *hovered, UNIT_ART_KIND kind)
{
    ...
    hovered->status_line[0] = '\0';
    hovered->has_status_effects = false;
}
                                                   ...
                                                   const Dungeon_Unit *unit)
{
    ...
    hovered->has_status_effects = game_dungeon_build_unit_status_text(unit, hovered->status_line,
...
static bool game_try_get_hovered_unit(Game *game, Rectangle floor_panel, Rectangle player_panel,
                                      Rectangle minimap_panel, Rectangle familiar_panel,
                                      Rectangle action_bar_panel, Dungeon_Hovered_Unit *out_hovered)
{
    ...
    if (game_mouse_is_over_dungeon_hud_panel(game, mouse_screen, floor_panel, player_panel,
                                             minimap_panel, familiar_panel, action_bar_panel)) {
        return false;
    }
...
static void game_draw_hovered_unit_tooltip(Game *game, Rectangle floor_panel,
                                           ...
                                           Rectangle familiar_panel, Rectangle action_bar_panel)
{
    ...
    if (!game_try_get_hovered_unit(game, floor_panel, player_panel, minimap_panel, familiar_panel,
    ...
    if (has_status_line) {
        status_measure = MeasureTextEx(game->font, status_line, status_size, game->font_spacing);
        panel_w = max(panel_w, status_measure.x);
    }
...
static void game_draw_test_dungeon(Game *game)
{
    ...
    for (u8 visual_idx = 0; visual_idx < DUNGEON_MAX_UNITS; visual_idx++) {
        const Dungeon_Defeated_Unit_Visual *visual = &game->defeated_unit_visuals[visual_idx];
        if (!visual->move_anim_active)
            continue;
        if (!game_dungeon_cell_is_visible(game, visual->x, visual->y))
            continue;

        Vector2 feet_position =
            game_dungeon_get_defeated_unit_visual_feet_position(visual, origin, tile_size);
    ...
    game_draw_tile_fx(game, origin, tile_size);
...
static bool game_update_player(Game *game)
{
    ...
    bool consumed_action_bar_click = game_player_try_activate_action_bar_slot_from_click(game);
    bool consumed_familiar_command_click = false;
    FAMILIAR_TURN_COMMAND clicked_familiar_command =
        game_player_get_familiar_command_from_click(game, &consumed_familiar_command_click);
    ...
    if (game_player_has_active_summon_target(game)) {
        if (consumed_action_bar_click || consumed_familiar_command_click)
            return false;
    ...
    bool requested_wait = game->input.pressed[INPUT_FAMILIAR_WAIT] ||
                          clicked_familiar_command == FAMILIAR_TURN_COMMAND_WAIT;
    if (requested_wait) {
        if (game_dungeon_player_should_skip_webbed_turn(game))
            return true;
    ...
    if (game_dungeon_has_living_familiar(game)) {
        bool requested_return = game->input.pressed[INPUT_FAMILIAR_RETURN] ||
                                clicked_familiar_command == FAMILIAR_TURN_COMMAND_RETURN;
        if (requested_return) {
            if (game_dungeon_player_should_skip_webbed_turn(game))
                return true;
    ...
    if (target_unit_idx >= 0) {
        Dungeon_Unit *target_unit = &game->units[target_unit_idx];
        if (target_unit->is_friendly) {
            game_dungeon_begin_unit_move_animation(target_unit, target_unit->x, target_unit->y);
            target_unit->x = (i16)start_x;
            target_unit->y = (i16)start_y;
            target_unit->orientation = game_dungeon_get_orientation_from_step(
                start_x - next_x, start_y - next_y, target_unit->orientation);
            target_unit->skip_friendly_turn_once = true;
        } else {
            float attack_delay = game_dungeon_get_unit_move_anim_remaining(target_unit);
            game_dungeon_begin_player_attack_animation(game, game->player_x, game->player_y, next_x,
                                                       next_y, attack_delay);
            target_unit->is_awake = true;
            target_unit->turns_out_of_player_los = 0;
...
static Rectangle game_draw_dungeon_minimap(Game *game)
{
    ...
    float screen_w = (float)GetScreenWidth();
    float screen_h = (float)GetScreenHeight();
    float minimap_width_fraction = DUNGEON_MINIMAP_SCREEN_WIDTH_FRACTION;
    if (screen_w <= 960.0f)
        minimap_width_fraction = 0.42f;
    if (screen_w <= 720.0f)
...
static void game_draw_debug_hud_button(Game *game, Rectangle button, const char *label, bool active)
{
    ...
    Color fill =
        active ? game_ui_hover_color(game_ui_gray_inner_color()) : game_ui_gray_inner_color();
    if (hovered)
        fill = game_ui_hover_color(fill);

    Color text = active ? (Color){244, 244, 244, 255} : (Color){224, 224, 224, 255};
...
{
    Rectangle floor_panel = game_draw_floor_info_panel(game);
    Rectangle player_panel = game_draw_player_stats_panel(game, floor_panel);
    ...
    game_draw_hovered_unit_tooltip(game, floor_panel, player_panel, minimap_panel, familiar_panel,
                                   action_bar_panel);
}
...
void game_update(Mem mem)
{
    ...
    game_dungeon_update_tile_fx(game);
